...
and the package folder once packaged; `build.ps1` and `package.ps1` copy
`static/` into both. `static/theme.css` holds the custom properties the widgets
read, which is the whole of the surface a page is meant to restyle - a widget's
own CSS lives in its shadow root and is not reachable from outside.

A request that tries to leave the directory is refused rather than cleaned:

```
neutrino://app/assets/../../secret     403
neutrino://app/assets/%2e%2e/secret    403     decoded before the check
neutrino://app/assets/C:/Windows/...   403
